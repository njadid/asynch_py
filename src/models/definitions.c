...
void SetParamSizes(GlobalVars* globals, void* external) {
	...
	case 401://tetis02
		...
		globals->area_idx = 0;
		globals->areah_idx = 2;
		globals->num_disk_params = 9;//read from the dbc file + linkid
		globals->convertarea_flag = 0;
		globals->num_forcings = 3;
		globals->min_error_tolerances = 8;
		break;
...
void ConvertParams(double *params, unsigned int model_uid, void* external) {
	if (model_uid == 19) {
		...
		params[2] *= 1e6;		//A_h: km^2 -> m^2
		params[4] *= .001;		//H_h: mm -> m
	} else if (model_uid == 252 || model_uid == 253 || model_uid == 254
			|| model_uid == 255 || model_uid == 256 || model_uid == 257
			|| model_uid == 400 || model_uid == 401 || model_uid == 260
			|| model_uid == 261 || model_uid == 262 || model_uid == 263) {
		params[1] *= 1000;		//L_h: km -> m
		params[2] *= 1e6;		//A_h: km^2 -> m^2
...
void InitRoutines(Link* link, unsigned int model_uid, unsigned int exp_imp,
		unsigned short dam, void* external) {
	//Select appropriate RK Solver for the numerical method (link->solver)
	if ((model_uid == 21 || model_uid == 22 || model_uid == 23
			|| model_uid == 40 || model_uid == 261 || model_uid == 262)
			&& dam == 1)
		link->solver = &ExplicitRKIndex1SolverDam;
	else if ((model_uid == 21 || model_uid == 22 || model_uid == 23
			|| model_uid == 40 || model_uid == 261 || model_uid == 262)
			&& dam == 0)
	...
	if (model_uid == 0) {
		...
		if (dam)
			link->differential = &dam_rain_hillslope;
		else
			link->differential = &nodam_rain_hillslope;
		...
		if (dam)
			link->differential = &dam_rain_hillslope2;
		else
			link->differential = &nodam_rain_hillslope2;
		...
		if (dam)
			link->differential = &dam_rain_hillslope3;
		else
			link->differential = &nodam_rain_hillslope3;
		...
		if (dam)
			link->differential = &dam_rain_hillslope_qsv;
		else
			link->differential = &nodam_rain_hillslope_qsv;
		...
		if (link->has_res) {
			...
			link->differential = &LinearHillslope_MonthlyEvap_extras;
		link->algebraic = NULL;
		link->check_state = NULL;
		...
		if (link->has_res) {
			...
			link->differential =
					&TopLayerNonlinearExpSoilvel_ConstEta_Reservoirs;
			link->algebraic = NULL;
			link->solver = &ForcedSolutionSolver;
		} else {
			link->num_dense = 2;
...
//unsigned int model_uid:		The index of the model.
void Precalculations(Link* link_i, double *global_params,
		unsigned int num_global_params, double *params,
		unsigned int num_disk_params, unsigned int num_params,
		unsigned short dam, unsigned int model_uid, void* external) {
	if (model_uid == 19) {
		...
		vals[4] = v_g * L_i / A_h * 60.0;	//[1/min]  k3
		vals[5] = pow(v_r * pow(A_i, lambda_2) / L_i, 1.0 / (1.0 - lambda_1))
				* 60.0; //[1/min]  invtau

		if (dam)
			vals[6] = 3.1415926535897932 * vals[11] * vals[11] / 4.0;
		...
		vals[8] = pow(v_r * pow(A_i, lambda_2) / L_i, 1.0 / (1.0 - lambda_1))
				* 60.0; //[1/min]  invtau

		if (dam)
			vals[9] = 3.1415926535897932 * vals[14] * vals[14] / 4.0; //orifice_area
	} else if (model_uid == 40) {
		...
		vals[14] = 60.0 * v_r * pow(A_i / A_r, lambda_2)
				/ ((1.0 - lambda_1) * L_i);	//invtau
		...
		params[12] = 60.0 * C_r * v_0 * pow(A_up / A_r, lambda_2)
				/ ((1.0 - lambda_1) * L_H);	//K_Q
		...
		//c_3
		vals[15] = 60.0 * 1e-6 * 2.0 / .6 * vals[0] / vals[1] * rootS_h
				/ vals[8];
	} else if (model_uid == 219) {
	...
	if (model_uid == 250) {
	...
	} else if (model_uid == 257) //stream order velocity
			{
		...
		double Hu = global_params[3]; //max available storage static storage [m]
		double infiltration = global_params[4]; //infiltration rate [mm/hr]
		double alfa2 = global_params[5]; //linear reservoir coef. surface storage [1/min]
		double percolation = global_params[6]; //percolation rate [mm/hr]
		double alfa3 = global_params[7]; //linear reserv. coef gravitational storage [1/min]
		double alfa4 = global_params[8]; //linear reserv. coef aquifer storage [1/min]
		...
		double L_i = params[1];	// channel lenght
		double A_h = params[2]; //area of the hillslope
		double Hu =  params[3];	//max available storage static storage [m]
		double infiltration = params[4]; //infiltration rate [m/min]
		double alfa2 = params[5]; //linear reservoir coef. surface storage [1/min]
		double percolation = params[6]; //percolation rate [m/min]
		double alfa3 = params[7]; //linear reserv. coef gravitational storage [1/min]
		double alfa4 = params[8]; //linear reserv. coef aquifer storage [1/min]


		double v_0 = global_params[0]; //velocity river in channels
		double lambda_1 = global_params[1]; //power discharge in routing function
		double lambda_2 = global_params[2]; //power of area in routing function
		double v_B = global_params[3]; //baseflow velocity [m/s]

		vals[9] = 60.0 * v_0 * pow(A_i, lambda_2) / ((1.0 - lambda_1) * L_i);//[1/min]  invtau params[9]
		//double v_h = 1;
		//vals[4] = v_h * L_i / A_h * 60.0;	// params[4]
		//double k_i_factor = 1;
		//vals[5] = vals[4] * k_i_factor;	//params[5]
		//vals[10] = (0.001 / 60.0);		//(mm/hr->m/min)  c_1
		vals[10] = A_h / 60.0;	//  c_2
...
int ReadInitData(double *global_params, unsigned int num_global_params,
		double *params, unsigned int num_params, QVSData* qvs,
		unsigned short int dam, double *y_0, unsigned int dim,
		unsigned int model_uid, unsigned int diff_start,
		unsigned int no_init_start, void* user, void* external) {
	unsigned int state;
	...
	if (model_uid == 19) {
		...
		state = dam_check(y_0, dim, global_params, num_global_params, params,
				num_params, qvs, dam, user);
		...
		state = dam_check2(y_0, dim, global_params, num_global_params, params,
				num_params, qvs, dam, user);
		...
		state = dam_check3(y_0, dim, global_params, num_global_params, params,
				num_params, qvs, dam, user);
		...
		state = dam_check_qvs(y_0, dim, global_params, num_global_params,
				params, num_params, qvs, dam, user);
		...
		if (dam) {
			int state = dam_check_qvs(y_0, dim, global_params,
					num_global_params, params, num_params, qvs, dam, user);
			dam_TopLayerNonlinearExpSoilvel(y_0, dim, global_params, params,
					qvs, state, user, y_0);
			return state;
		} else {
			double lambda_1 = global_params[1];
