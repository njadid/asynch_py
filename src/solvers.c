...
//printf("setting forcing maxtime to %e, iteration = %u\n",forcings[i].maxtime,forcings[i].iteration);
        ...
        if (GlobalVars->res_flag)
        {
            for (i = 0; i < my_N; i++)	//!!!! Can we loop over just the links with reservoirs? Improve id_to_loc. !!!!
            {
                ...
                if (current->res)
                {
                    current->f(current->last_t, current->list->tail->y_approx, NULL, current->num_parents, GlobalVars->global_params, current->forcing_values, current->qvs, current->params, current->state, current->user, current->list->tail->y_approx);
                ...
                else	//Compute an iteration
                {
                    ...
                    if (current->current_iterations < GlobalVars->iter_limit)
                    {
                        ...
                        else	//Has parents
                        {
                            ...
                            if (parentsval == current->num_parents && current->current_iterations < GlobalVars->iter_limit && current->last_t < maxtime)		//If all parents are done, then current should finish up too
                            {
                                current->h = min(current->h, maxtime - current->last_t);
                                assert(current->h > 0);
                                for (i = 0; i < GlobalVars->num_forcings; i++)
                        ...
                        if (child != NULL && assignments[child->location] == my_rank)
                        {
                            ...
                            if ((current->current_iterations >= GlobalVars->iter_limit) && (current->last_t > child->last_t))
                        ...
                        for (i = 0; i < current->num_parents; i++)
                            parentsval += (current->last_t + current->h <= current->parents[i]->last_t);
                        if (parentsval == current->num_parents && current->current_iterations < GlobalVars->iter_limit)
                            current->ready = 1;
