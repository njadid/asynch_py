...
int DataDumpH5(Link** sys, unsigned int N, int* assignments, UnivVars* GlobalVars, char* suffix, ConnData* conninfo)
{
    ...
    if (my_rank == 0)	
    {
        ...
        if (suffix)
        {
            ...
            char *underscore = strrchr(GlobalVars->dump_loc_filename, '_');
            if (underscore != NULL)
            {
                char filename[ASYNCH_MAX_PATH_LENGTH];
                char timestamp[ASYNCH_UNIX_TIMESTAMP_LENGTH + 1];
                unsigned int filename_len = underscore - GlobalVars->dump_loc_filename;

                // Get the base name from the global file. Parse the filename (the part before underscore)
                strncpy(filename, GlobalVars->dump_loc_filename, filename_len);
                filename[filename_len] = '\0';
                strncpy(timestamp, underscore + 1, ASYNCH_UNIX_TIMESTAMP_LENGTH);
                timestamp[ASYNCH_UNIX_TIMESTAMP_LENGTH] = '\0';
                char *endptr;
                int start_timestamp = atoi(timestamp);
                if (start_timestamp == 0)
                {
                    printf("Error: Snapshot file name %s not standard: filename_timestamp.h5", GlobalVars->dump_loc_filename);
