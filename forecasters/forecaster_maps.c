...
int main(int argc, char* argv[])
{
    ...
    //Asynch_Set_Output_Int(asynch, "LinkID", &Output_Linkid, NULL, 0);
    ...
    while (!halt)
    {
        ...
        if (my_rank == 0)
        {
            PerformTableMaintainance(&asynch->db_connections[ASYNCH_DB_LOC_HYDRO_OUTPUT], asynch->globals, Forecaster, &vac_hydros, hr1, num_tables, "archive_hydroforecast", schema);
            PerformTableMaintainance(&asynch->db_connections[ASYNCH_DB_LOC_PEAK_OUTPUT], asynch->globals, Forecaster, &vac_peakflows, hr1, num_tables, "archive_peakflows", schema);
            PerformTableMaintainance(&asynch->db_connections[ASYNCH_DB_LOC_SNAPSHOT_OUTPUT], asynch->globals, Forecaster, &vac_maps, hr1, num_tables, "archive_maps", schema);
        }
