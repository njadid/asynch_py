...
int Output_Timestamp(unsigned int id, double t, VEC y_i, VEC global_params, VEC params, int state, void* user);
...
int main(int argc, char* argv[])
{
    ...
    Asynch_Set_Total_Simulation_Duration(asynch, (atoi(argv[4]) - atoi(argv[3])) / 60.0);
    ...
    double holder = Asynch_Get_Total_Simulation_Duration(asynch);
    ...
    unsigned int N = Asynch_Get_Num_Links(asynch);
    unsigned int my_N = Asynch_Get_Num_Links_Proc(asynch);
    ...
    for (i = 0; i < asynch->globals->num_forcings; i++)
        Asynch_Set_First_Forcing_Timestamp(asynch, override_starttime, i);
    Asynch_Set_Last_Forcing_Timestamp(asynch, override_endtime, forecast_idx);
    ...
    if (my_rank == 0)
    {
        ...
        Print_Vector(asynch->globals->global_params);
        printf("\n");
    }
    ...
    size_t place, tablename_len = strlen(asynch->globals->hydro_table);
    char schema[128]; schema[0] = '\0';
    for (place = tablename_len - 1; place > -1; place--)
    {
        if (asynch->globals->hydro_table[place] == '.')
        {
            asynch->globals->hydro_table[place] = '\0';
            strcpy(schema, asynch->globals->hydro_table);
            asynch->globals->hydro_table[place] = '.';
            schema[place] = '.';
            schema[place + 1] = '\0';
            break;
        }
    ...
    Asynch_Set_Total_Simulation_Duration(asynch, forecast_time);
    ...
    if (my_rank == 0)
    {
        ...
        if (!hydro_files)
        {
            ...
            CheckPartitionedTable(&asynch->db_connections[ASYNCH_DB_LOC_HYDRO_OUTPUT], asynch->globals, Forecaster, num_tables, "archive_hydroforecast", "forecast_time", schema);

            //Clear the future hydrographs in archive
            DeleteFutureValues(&asynch->db_connections[ASYNCH_DB_LOC_HYDRO_OUTPUT], num_tables, asynch->globals, "archive_hydroforecast", Forecaster->model_name, first_file, 1, schema);
        }
        ...
        CheckPartitionedTable(&asynch->db_connections[ASYNCH_DB_LOC_PEAK_OUTPUT], asynch->globals, Forecaster, num_tables, "archive_peakflows", "forecast_time", schema);

        //Clear all future peakflows
        DeleteFutureValues(&asynch->db_connections[ASYNCH_DB_LOC_PEAK_OUTPUT], num_tables, asynch->globals, "archive_peakflows", Forecaster->model_name, first_file, 1, schema);
        ...
        CheckPartitionedTable(&asynch->db_connections[ASYNCH_DB_LOC_SNAPSHOT_OUTPUT], asynch->globals, Forecaster, num_tables, "archive_maps", "forecast_time", schema);

        //Clear all future maps
        DeleteFutureValues(&asynch->db_connections[ASYNCH_DB_LOC_SNAPSHOT_OUTPUT], num_tables, asynch->globals, "archive_maps", Forecaster->model_name, first_file, 0, schema);
    ...
    while (!halt)
    {
        ...
        if (isnull)
        {
            if (my_rank == 0)
            {
                ...
                if (!hydro_files)	CheckPartitionedTable(&asynch->db_connections[ASYNCH_DB_LOC_HYDRO_OUTPUT], asynch->globals, Forecaster, num_tables, "archive_hydroforecast", "forecast_time", schema);
                CheckPartitionedTable(&asynch->db_connections[ASYNCH_DB_LOC_PEAK_OUTPUT], asynch->globals, Forecaster, num_tables, "archive_peakflows", "forecast_time", schema);
                CheckPartitionedTable(&asynch->db_connections[ASYNCH_DB_LOC_SNAPSHOT_OUTPUT], asynch->globals, Forecaster, num_tables, "archive_maps", "forecast_time", schema);
            }
        ...
        Asynch_Set_Total_Simulation_Duration(asynch, simulation_time_with_data);		// !!!! This may not work for multiple forcings for forecasting. How do you handle different time resolutions? !!!!
        ...
        if (snapshot_files)	//See if a .rec file should be uploaded created and uploaded somewhere
        {
            ...
            if (my_rank == 0)
            {
                while (SendFilesTo51(asynch->globals->dump_loc_filename, "/data/ifc_01_maps/"))
                {
        ...
        for (i = 0; i < num_future_peakflow_times; i++)
        {
            ...
            Asynch_Set_Total_Simulation_Duration(asynch, future_peakflow_times[i] + db_stepsize*num_rainsteps);
        ...
        {
            sprintf(query, "%s_%s_%i.irad", asynch->globals->hydros_loc_filename, hydro_additional, my_rank);
            ...
            sprintf(query, "%s_%s_%i.rad", asynch->globals->hydros_loc_filename, hydro_additional, my_rank);
            while (SendFilesTo51(query, snapshot_file_location))
            {
